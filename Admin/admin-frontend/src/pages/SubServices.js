...
    try {
      const res = await axiosClient.put("/subservice/update_subservice", payload);
      alert(res.data);
      clearForm();
      loadSubServices();
    } catch (error) {
...
    if (s.image) {
      setImagePreview(`data:image/jpeg;base64,${s.image}`);
...
    try {
      const res = await axiosClient.delete(`/subservice/delete_subservice/${id}`);
      alert(res.data);
      loadSubServices();
    } catch (error) {
...
          {isEdit ? (
            <>
              <button className="btn" onClick={updateSubService}>Update</button>
              &nbsp;
              <button className="btn-danger" onClick={clearForm}>Cancel</button>
            </>
          ) : (
            <button className="btn" onClick={addSubService}>Add</button>
          )}
        </div>
...
                </td>
                <td>
                  <button className="btn" onClick={() => editSubService(s)}>Edit</button>
                  &nbsp;
                  <button className="btn-danger" onClick={() => deleteSubService(s.serviceId)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
